...
table.n1 <-
        data.table::data.table(as.data.frame(sample.n1) %>% separate(text, into = paste("w", 1, sep = ""), remove = FALSE))
...
## Split text into separate words
table.n2 <-
        data.table::data.table(as.data.frame(sample.n2) %>% separate(text, into = paste("w", 1:2, sep = ""), remove = FALSE))
...
## Split text into separate words
table.n3 <-
        data.table::data.table(as.data.frame(sample.n3) %>% separate(text, into = paste("w", 1:3, sep = ""), remove = FALSE))
...
## Split text into separate words
table.n4 <-
        data.table::data.table(as.data.frame(sample.n4) %>% separate(text, into = paste("w", 1:4, sep = ""), remove = FALSE))
